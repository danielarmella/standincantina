...
class User(AbstractUser):
    ...
    is_stand_in = models.BooleanField('Stand-in', default=False)    # Checked if the user is a stand-in
...
class StandIn(models.Model):
    user = models.OneToOneField(User, on_delete=models.CASCADE, related_name='stand_in')
    ...
    reviews = models.ManyToManyField(AD, related_name="reviewed_stand_ins", through="Review")
...
class HairColor(models.Model):
    stand_in = models.ForeignKey(StandIn, on_delete=models.CASCADE, related_name="hair_colors")
...
class Incident(models.Model):
    stand_in = models.ForeignKey(StandIn, on_delete=models.CASCADE)
...
class ActorStandInMatch(models.Model):
    actor = models.ForeignKey(Actor, on_delete=models.CASCADE, related_name="stand_in_matches")
    stand_in = models.ForeignKey(StandIn, on_delete=models.CASCADE, related_name="actor_matches")

    def __str__(self):
        return f'{self.stand_in.user.first_name} {self.stand_in.user.last_name} | {self.actor.first_name} {self.actor.last_name}'

    class Meta:
        constraints = [
            models.UniqueConstraint(
                fields=['actor', 'stand_in'],
                name='unique_actor_stand_in_match',
...
class Availability(models.Model):
    stand_in = models.ForeignKey(StandIn, on_delete=models.CASCADE, related_name="avails")
    ...
    def clean(self):
        ...
        if self.booked:
            ...
            if self.stand_in != self.booked.stand_in:
...
class Booking(models.Model):
    stand_in = models.ForeignKey(StandIn, on_delete=models.CASCADE, related_name="stand_in_bookings")
    ...
    def __str__(self):
        return f'{self.start_date} - {self.end_date} | {self.stand_in} on {self.project.name}'
    ...
    def resolve_overlaps(self):
        ...
        if overlapping_avails:
            ...
            return False, f"{self.stand_in} is unavailable on the following conflicting dates:\n{unavail}"
        ...
        for entry in overlapping_entries:
            if entry.project == self.project:
                ...
                return False, f"{self.stand_in} is already booked from {entry.start_date} to {entry.end_date} for {entry.project.name}."
                ...
                try:
                    ...
                    if old_booking.start_date != self.start_date or old_booking.end_date != self.end_date:
                        ...
                        try:
                            ...
                            old_avail = Availability.objects.create(stand_in=old_booking.stand_in, start_date=old_booking.start_date, end_date=old_booking.end_date, is_available=True, booked=None, notes=f"Booking for {old_booking.project} canceled or updated. Stand-in now available.")
...
class AvailCheck(models.Model):
    stand_ins = models.ManyToManyField(StandIn, blank=True, related_name='avail_checks')
    ...
    def send_email_notifications(self):
        ...
        for stand_in in self.stand_ins.all():
            accept_url = f"<a href='127.0.0.1:8000{reverse("cantina:accept_availability", args=[self.id, stand_in.id])}'>Accept</a>"
            reject_url = f"<a href='127.0.0.1:8000{reverse('cantina:reject_availability')}'>Reject</a>"

            message = f"""
            Hi {stand_in.user.first_name},
