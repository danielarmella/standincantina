        ...
        nav_link.addEventListener('click', (event) => {
            ...
            switch (nav_link.dataset.view) {
                ...
                case 'register_user':
                    load_register_user();
                    break;
                case 'register_stand_in':
                    load_register_stand_in();
                    break;
...
function view_switcher(view) {
    ...
    if (!['home', 'user_account', 'Stand-in_profile', 'register_user', 'register_stand_in', 'registration_pending', 'login', 'logout'].includes(view)) {
...
function fixStandInCheckbox() {
    // fix is_stand_in check box
    const is_stand_in_input = document.querySelector('#id_is_stand_in');
    if (!is_stand_in_input) return;

    const is_stand_in_div = is_stand_in_input.parentElement;
    is_stand_in_div.classList.add('checkbox_parent')

    const new_div = document.createElement('div');
    new_div.classList.add('checkbox_label_div');

    const childrenArray = Array.from(is_stand_in_div.children);

    childrenArray.forEach(child => {
        if (child !== is_stand_in_input) {
            new_div.appendChild(child);
        }
    });
    is_stand_in_div.innerHTML= '';
    is_stand_in_div.append(is_stand_in_input);
    is_stand_in_div.append(new_div);
}
...
async function handle_regUser_btn() {
    ...
    const message = document.querySelector(".message");
    const is_stand_in = document.getElementsByName('is_stand_in')[0].checked;
    ...
    await fetch(`register_user`, {
        ...
        .then(result => {
            ...
            if (result.status === 'success') {
                if (is_stand_in) {
                    load_register_stand_in(result.user);
                } else {
                    load_reg_pending();
                }
            }
...
async function load_register_stand_in(user) {
    document.querySelector('#pHeading').innerHTML = 'Stand-in registration';
    let view = document.querySelector('#register_stand_in-view');
    view.innerHTML = `
        <form>
    ...
    try {
        let response = await fetch(`register_stand_in/${user.id}`, {
            method: 'GET',
            headers: {
                'content-type': 'application/json',
            }
        ...
        if (data.stand_in_reg_form) {
            standInRegistration_form.innerHTML = data.stand_in_reg_form;
        } else {
            console.error("Error: stand_in_reg_form is missing in response.");
        }
    } catch (error) {
    ...
    // Switch to the register_stand_in view
    view_switcher('register_stand_in');
}
...
async function handle_regStandIn_btn(user) {
    ...
    await fetch(`register_stand_in/${user.id}/`, {
            method: 'POST',
            headers: {
                'X-CSRFToken': getCSRFToken(),
            },
