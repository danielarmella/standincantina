...
class UserRegistrationForm(UserCreationForm):
    ...
    class Meta:
        model = User
        fields = ['first_name', 'last_name', 'phone', 'email', "password1", "password2",'birthday', "is_stand_in"]
        widgets = {
            ...
            'is_stand_in': forms.CheckboxInput()
        }
        help_texts = {
            ...
            'is_stand_in': '<ul><li>Check to register as a stand-in.</li><li>Leave unchecked to register as staff only</li></ul>'
        }
    ...
    def serialize(self):
        ...
        return json.dumps({
            ...
            "is_stand_in": self.cleaned_data.get("is_stand_in", False),
        }, indent=4)
...
class IncidentForm(forms.ModelForm):
    class Meta:
        model = Incident
        fields = ['stand_in', 'complainant', 'incident', 'note', 'needs_followup']
        widgets = {
...
class ReviewForm(forms.ModelForm):
    class Meta:
        model = Review
        fields = ['stand_in', 'ad', 'feedback', 'is_positive']
        widgets = {
...
class DNRForm(forms.ModelForm):
    class Meta:
        model = DNR
        fields = ['stand_in', 'ad', 'project', 'reason']
        widgets = {
...
class AvailabilityForm(forms.ModelForm):
    class Meta:
        model = Availability
        fields = ['stand_in', 'start_date', 'end_date']
        widgets = {
            'start_date': forms.DateInput(attrs={'type': 'date'}),
            'end_date': forms.DateInput(attrs={'type': 'date'}),
        }
...
class BookingForm(forms.ModelForm):
    class Meta:
        model = Booking
        fields = ['stand_in', 'project', 'start_date', 'end_date', 'email_reminder_sent']
        widgets = {
            'start_date': forms.DateInput(attrs={'type': 'date'}),
            'end_date': forms.DateInput(attrs={'type': 'date'}),
        }
...
#     # Replace the stand_in field with a multiple choice field.
#     stand_ins = forms.ModelMultipleChoiceField(
#         queryset=StandIn.objects.all(),
...
#     class Meta:
#         model = AvailCheck
#         fields = ['stand_ins', 'project', 'start_date', 'end_date']
#         widgets = {
#             'project': AutocompleteSelect(AvailCheck._meta.get_field('project').remote_field, admin.site),
